...
│                      **Application Layer**                      │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐ │
│  │ **User Code**   │  │ **Goroutines**  │  │ **Go Runtime**  │ │
│  │                 │  │                 │  │                 │ │
│  │ • **Functions** │  │ • **go func()** │  │ • **Scheduler** │ │
│  │ • **Methods**   │  │ • **Channels**  │  │ • **GC**        │ │
│  │ • **Logic**     │  │ • **Select**    │  │ • **Memory**    │ │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘ │
└─────────────────────┬───────────────────┬───────────────────────┘
...
│  │                                                           │ │
│  │  ┌─────────────────┐    ┌─────────────────────────────┐  │ │
│  │  │**Global RunQ**  │    │     **Scheduler State**     │  │ │
│  │  │                 │    │                             │  │ │
│  │  │ • **FIFO Queue**│    │ • **GOMAXPROCS**            │  │ │
│  │  │ • **Mutex Lock**│    │ • **P Count**               │  │ │
│  │  │ • **Load Balance**  │ • **M Count**               │  │ │
│  │  │ • **Work Stealing** │ • **Idle List**             │  │ │
│  │  └─────────────────┘    └─────────────────────────────┘  │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
│  ┌───────────────────────────────────────────────────────────┐ │
...
│ │                 │ │                 │ │                 │  │
│ │ • **Active G**  │ │ • **Ready to Run**│ • **Waiting**   │  │
│ │ • **On M Thread**│ │ • **In RunQ**   │ │ • **Channel**   │  │
│ │ • **Executing** │ │ • **Waiting CPU**│ │ • **Sleep**     │  │
│ │ • **Stack Active**│ │ • **Preempted** │ │ • **I/O**       │  │
│ └─────────────────┘ └─────────────────┘ └─────────────────┘  │
│                                                                 │
│ ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐  │
│ │    **Dead**     │ │   **Syscall**   │ │ **Preempted**  │  │
│ │                 │ │                 │ │                 │  │
│ │ • **Finished**  │ │ • **OS Syscall**│ │ • **Time Slice**│  │
│ │ • **Panic**     │ │ • **CGO Call**  │ │ • **Cooperative**│ │
│ │ • **GC Ready**  │ │ • **Blocking**  │ │ • **Force Stop** │  │
│ └─────────────────┘ └─────────────────┘ └─────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
│                  │    │                  │    │                  │
│ • **Stack**      │    │ • **OS Thread** │    │ • **Local RunQ** │
│ • **PC/SP**      │    │ • **G Pointer** │    │ • **M Pointer**  │
│ • **State**      │    │ • **P Pointer** │    │ • **Status**     │
│ • **ID**         │    │ • **Stack Ptr** │    │ • **mcache**     │
│ • **Park/Ready** │    │ • **Registers** │    │ • **Timers**     │
└──────────────────┘    └──────────────────┘    └──────────────────┘
         │                        │                        │
         └────────────────────────┼────────────────────────┘
                                  │
...
```text
                ...
                ▼                 ▼                 ▼
    ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
    │ **Syscall**     │ │ **Channel Ops** │ │ **Preempted**  │
    │ **_Gsyscall**   │ │ **_Gwaiting**   │ │ **_Grunnable**  │
    └─────────┬───────┘ └─────────┬───────┘ └─────────┬───────┘
              │                   │                   │
              ▼                   ▼                   │
    ┌─────────────────┐ ┌─────────────────┐           │
    │**Exit Syscall** │ │**Channel Ready**│           │
...
```text
                        ...
                        └─────────┬───────┘
                                  ...
                                  ▼
                        ┌─────────────────┐      **Found**   ┌─────────────────┐
                        │**Check Global** │─────────────────►│ **Get from**    │
                        │**RunQ**         │                  │ **Global RunQ** │
                        └─────────┬───────┘                  └─────────────────┘
                                  │ **Empty**
                                  ▼
                        ┌─────────────────┐      **Found**   ┌─────────────────┐
                        │**Check NetPoll**│─────────────────►│ **Get Network** │
                        │**Events**       │                  │ **Ready G**     │
                        └─────────┬───────┘                  └─────────────────┘
                                  │ **No Events**
                                  ▼
                  ...
                  ▼                               ▼
        ┌─────────────────┐               ┌─────────────────┐
        │**Try Lock**     │               │**Check Target** │
        │**Target RunQ**  │               │**Queue Size**   │
        └─────────┬───────┘               └─────────┬───────┘
                  │ **Success**                     │
                  ▼                                 ▼
        ┌─────────────────┐               ┌─────────────────┐
        │**Calculate**    │               │**Steal Half**   │
        │**Steal Amount** │               │**+ Random**     │
        └─────────┬───────┘               └─────────┬───────┘
                  │                                 │
                  └─────────────┬───────────────────┘
                                ...
                                ▼
                      ┌─────────────────┐      **Success** ┌─────────────────┐
                      │**Atomic Steal** │─────────────────►│ **Return G**    │
                      │**CAS Operation**│                  │ **Execute**     │
                      └─────────┬───────┘                  └─────────────────┘
                                │ **Failed**
                                ▼
                      ┌─────────────────┐
