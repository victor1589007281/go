...
│                      **Application Layer**                      │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐ │
│  │ **HTTP Client** │  │ **File System** │  │ **Network I/O** │ │
│  │                 │  │                 │  │                 │ │
│  │ • **Request**   │  │ • **Open/Read** │  │ • **TCP/UDP**   │ │
│  │ • **Response**  │  │ • **Write/Close**│ │ • **WebSocket** │ │
│  │ • **JSON/XML**  │  │ • **Seek**      │  │ • **gRPC**      │ │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘ │
└─────────────────────┬───────────────────┬───────────────────────┘
                      │                   │
...
│  │                                                           │ │
│  │  ┌─────────────────┐    ┌─────────────────────────────┐  │ │
│  │  │**ReadSeeker**   │    │   **WriteSeeker**           │  │ │
│  │  │                 │    │                             │  │ │
│  │  │ • **Reader**    │    │ • **Writer + Seeker**      │  │ │
│  │  │ • **Seeker**    │    │ • **File Operations**       │  │ │
│  │  └─────────────────┘    └─────────────────────────────┘  │ │
│  └───────────────────────────────────────────────────────────┘ │
└─────────────────────┬───────────────────┬───────────────────────┘
                      │                   │
...
│  │  │                 │    │                             │  │ │
│  │  │ • **Buffer Mgmt**│   │ • **Write Buffering**       │  │ │
│  │  │ • **Peek/Read** │    │ • **Flush Control**         │  │ │
...
│  │  │ **Scanner**     │    │    **Pipe**                 │  │ │
│  │  │                 │    │                             │  │ │
│  │  │ • **Token Split**│   │ • **PipeReader**            │  │ │
│  │  │ • **Text Proc** │    │ • **PipeWriter**            │  │ │
│  │  │ • **Line-by-Line**  │ • **Goroutine Safe**        │  │ │
│  │  └─────────────────┘    └─────────────────────────────┘  │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
│  ┌───────────────────────────────────────────────────────────┐ │
...
│  │                                                           │ │
│  │  ┌─────────────────┐    ┌─────────────────────────────┐  │ │
│  │  │ **Copy/CopyN**  │    │   **MultiReader/Writer**    │  │ │
│  │  │                 │    │                             │  │ │
│  │  │ • **Streaming** │    │ • **Composition**           │  │ │
│  │  │ • **Efficient** │    │ • **Chain Operations**      │  │ │
│  │  │ • **Buffer Mgmt**│   │ • **Sequential Access**     │  │ │
│  │  └─────────────────┘    └─────────────────────────────┘  │ │
│  └───────────────────────────────────────────────────────────┘ │
└─────────────────────┬───────────────────┬───────────────────────┘
                      │                   │
...
│                                                                 │
│ ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐  │
│ │  **File I/O**   │ │ **Network I/O** │ │  **Memory I/O** │  │
│ │                 │ │                 │ │                 │  │
│ │ • **os.File**   │ │ • **net.Conn**  │ │ • **bytes.Buffer**│ │
│ │ • **Syscalls**  │ │ • **Sockets**   │ │ • **strings.Reader**│ │
│ │ • **Buffering** │ │ • **Protocols** │ │ • **In-Memory** │  │
│ └─────────────────┘ └─────────────────┘ └─────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
│                  │    │                  │    │                  │
│ • **Memory**     │    │ • **String**     │    │ • **Buffered**   │
│ • **Seek**       │    │ • **Immutable**  │    │ • **Peek/Read**  │
│ • **Reset**      │    │ • **Efficient**  │    │ • **ReadLine**   │
└──────────────────┘    └──────────────────┘    └──────────────────┘
         │                        │                        │
         └────────────────────────┼────────────────────────┘
                                  │
         ...
         ▼                        ▼                        ▼
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│**bytes.Buffer**  │    │ **bufio.Writer** │    │ **MultiWriter**  │
│                  │    │                  │    │                  │
│ • **Dynamic**    │    │ • **Buffered**   │    │ • **Broadcast**  │
│ • **ReadWrite**  │    │ • **Flush**      │    │ • **Fan-out**    │
│ • **Memory Pool**│    │ • **Efficient**  │    │ • **Replication**│
└──────────────────┘    └──────────────────┘    └──────────────────┘
         │                        │                        │
         └────────────────────────┼────────────────────────┘
                                  │
...
```text
                        ...
                        └─────────┬───────┘
                                  ...
                                  ▼
                        ┌─────────────────┐      **Buffered** ┌─────────────────┐
                        │**Check Buffer** │─────────────────►│ **bufio.Reader** │
                        │**Data Available**│                  │ **Fill Buffer**  │
                        └─────────┬───────┘                  └─────────┬───────┘
                                  │ **Direct**                         │
                                  ▼                                    ▼
                        ┌─────────────────┐                  ┌─────────────────┐
                        │**Direct Read**  │                  │**Copy from Buf**│
                        │**io.Reader**    │                  │**Return Data**  │
                        └─────────┬───────┘                  └─────────────────┘
                                  │
                                  ▼
                  ┌───────────────┴───────────────┐
            ...
            │                 │ │                │ │                 │
            │ • **os.File**   │ │ • **net.Conn** │ │ • **bytes.Buffer** │
            │ • **Syscall**   │ │ • **Socket**   │ │ • **strings.Reader** │
            │ • **read()**    │ │ • **recv()**   │ │ • **Memory Copy** │
            └─────────┬───────┘ └────────┬───────┘ └─────────┬───────┘
                      │                  │                   │
                      └──────────────────┼───────────────────┘
                                         │
...
```text
                        ...
                        └─────────┬───────┘
                                  ...
                                  ▼
                        ┌─────────────────┐      **Buffered** ┌─────────────────┐
                        │**Check Buffer** │─────────────────►│**bufio.Writer** │
                        │**Size & Type**  │                  │**Buffer Data**  │
                        └─────────┬───────┘                  └─────────┬───────┘
                                  │ **Direct**                         │
                                  ▼                                    ▼
                        ┌─────────────────┐                  ┌─────────────────┐
                        │**Direct Write** │                  │**Check if Full**│
                        │**io.Writer**    │                  │**Auto Flush**  │
                        └─────────┬───────┘                  └─────────┬───────┘
                                  │                                    │
                                  ▼                                    ▼
                  ┌───────────────┴───────────────┐         ┌─────────────────┐
                  │ **Implementation Type**       │         │**Flush Buffer** │
                  └─────┬─────────┬─────────┬─────┘         │**Write to Dest**│
                        │         │         │               └─────────────────┘
                        ▼         ▼         ▼
            ┌─────────────────┐ ┌────────────────┐ ┌─────────────────┐
            ...
            │                 │ │                │ │                 │
            │ • **os.File**   │ │ • **net.Conn** │ │ • **bytes.Buffer** │
            │ • **Syscall**   │ │ • **Socket**   │ │ • **Memory Alloc** │
            │ • **write()**   │ │ • **send()**   │ │ • **Memory Grow** │
            └─────────┬───────┘ └────────┬───────┘ └─────────┬───────┘
                      │                  │                   │
                      └──────────────────┼───────────────────┘
                                         │
...
```text
                        ...
                        └─────────┬───────┘
                                  ...
                                  ▼
                        ┌─────────────────┐      **Yes**   ┌─────────────────┐
                        │**Check WriterTo**│─────────────►│**src.WriteTo** │
                        │**Interface**    │               │**Direct Copy**  │
                        └─────────┬───────┘               └─────────┬───────┘
                                  │ **No**                          │
                                  ▼                                 │
                        ┌─────────────────┐      **Yes**           │
          ...
          ┌─────────────────┐         ┌─────────────────┐     │
          │ **src.Read**    │         │ **Check EOF**   │     │
          │ **Fill Buffer** │         │ **or Error**    │     │
          └─────────┬───────┘         └─────────┬───────┘     │
                    │ **Success**               │ **Done**    │
                    ▼                           ▼             │
          ┌─────────────────┐         ┌─────────────────┐     │
          │ **dst.Write**   │         │ **Return Total**│     │
          │ **Flush Buffer**│         │ **Bytes & Error**    │
          └─────────┬───────┘         └─────────┬───────┘     │
                    │                           │             │
                    └───────────┬───────────────┘             │
                                │                             │
...
```text
    ...
    └─────────┬───────┘               └─────────┬───────┘
              ...
              ▼                                 ▼
    ┌─────────────────┐               ┌─────────────────┐
    │**Write to Buf** │               │**Read from Buf**│
    │**Signal Reader**│               │**Signal Writer**│
    └─────────┬───────┘               └─────────┬───────┘
              │                                 │
              └─────────────┬───────────────────┘
                            │
